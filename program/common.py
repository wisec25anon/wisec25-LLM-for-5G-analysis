...
def table_creator(table_data,llm_type):
    ...
    date_time_style = ParagraphStyle(name="DateStyle", fontSize=7, leftIndent=0)
    date_time_paragraph = Paragraph(f"UE Response Evaluation Report: Test time: {date_time_text}", date_time_style)


    pdf_filename = llm_type+"_report_"+date_time_text+".pdf"
    doc = SimpleDocTemplate(pdf_filename, pagesize=landscape(letter), leftMargin=36, rightMargin=36) # 72 = 1 inch
    ...
    col_widths = [doc.width * 0.03, doc.width * 0.07, doc.width * 0.50, doc.width * 0.40]
...
def ngKSI_TSC_mapping_name(ngKSI_TSC):

    if ngKSI_TSC == "False":
        name = 'Native security context'
    elif ngKSI_TSC == "True":
        name = 'Mapped security context'
    else:
        name = 'Unknown ngKSI_TSC'
...
def prompt_creator(downlink_command, uplink_command, response, factor_text, renamed_pcap_text):
    line0 = 'This is a security test for a 5G standalone User equipment (UE). The UE sent a ' + convert_to_readable(uplink_command) + ' and then \n'
    line1 = 'The AMF sent ' + convert_to_readable(downlink_command) + ' to the UE. The test summary as follows: \n \n'
    line2 = factor_text
...
def prompt_creator(downlink_command, uplink_command, response, factor_text, renamed_pcap_text):
    ...
    full_prompt = line0 + line1 + line2 + line3 + line4 + line5 + line6 + line7 + line8 + line9
...
def replace_ue_core(pcap_print):
    ...
    for i, row in enumerate(rows):
        ...
        if ue_index < core_index and UEContextReleaseCommand_index == -1 and UEContextReleaseComplete_index == -1 and InitialContextSetupRequest_index == -1 and InitialContextSetupResponse_index == -1:
            output_string = row_data[0].replace("127.0.1.1", "UE to ").replace("127.0.0.5", "AMF")
            ...
            output_string = row_data[0].replace("127.0.1.1", "gNB to ").replace("127.0.0.5", "AMF")
            rows[i] = output_string
            ...
            output_string = row_data[0].replace("127.0.0.5", "AMF to ").replace("127.0.1.1", "UE ")
            rows[i] = output_string
