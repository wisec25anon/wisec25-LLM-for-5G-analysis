import re
from program.common import convert_to_readable , cipher_algorithm_mapping_name, integrity_algorithm_mapping_name, get_tshark_output, prompt_creator, ngKSI_TSC_mapping_name 
...
def authentication_request_factor_retrieve_text(uplink_command,downlink_command,key_estbl_status,message_encoding_type,pcap_file_path,dl_com_invoke_line):
    ...
    if ngksi_tsc is not None and ngksi_tsc.strip():
        ngksi_tsc_value = ngKSI_TSC_mapping_name(str(ngksi_tsc))
...
def authntication_request_part(factor_to_consider, uplink_command, downlink_command ,response, pcap_file_path,dl_com_invoke_line, renamed_pcap_text,llm_type):
