...
def main(pcap_directory,testcase_directory, mode):

    # Check if the directories exist
    if not os.path.exists(pcap_directory):
        print(f"Error: The directory '{pcap_directory}' does not exist.")
        sys.exit(1)
    if not os.path.exists(testcase_directory):
        print(f"Error: The directory '{testcase_directory}' does not exist.")
        sys.exit(1)
    
    # Check if the directories are empty
    if not os.listdir(pcap_directory):
        print(f"Error: The directory '{pcap_directory}' is empty.")
        sys.exit(1)
    if not os.listdir(testcase_directory):
        print(f"Error: The directory '{testcase_directory}' is empty.")
        sys.exit(1)
        

    if mode == "chatgpt":
        print("Mode 'chatgpt' selected. Performing tasks for ChatGPT...")
        llm_type = 'chatgpt'
    elif mode == "gemini":
        print("Mode 'gemini' selected. Performing tasks for Gemini...")
        llm_type = 'gemini'
    else:
        print(f"Error: Invalid mode '{mode}'. Please select 'chatgpt' or 'gemini'.")
        sys.exit(1)
        
        

    table_data = [["SL","Test Name", 'Test content', "Remark"]]
        ...
        if path_exists:
            ...
            if result is not None and result_2 is not None:
                ...
                if  (downlink_command.lower() == 'identity_request'):
                    ...
                    full_prompt, llm_answer, status = Identity_Request.identity_request_part(factor_to_consider, uplink_command, downlink_command, response, 
                    pcap_file_path,dl_com_invoke_line, renamed_pcap_text,llm_type)
                    
                elif  (downlink_command.lower() == 'authentication_request'):
                    response = common.handle_response(pattern,ue_response)
                    full_prompt, llm_answer, status = Authentication_Request.authntication_request_part(factor_to_consider, uplink_command, downlink_command, 
                    response, pcap_file_path,dl_com_invoke_line,renamed_pcap_text,llm_type)
                
                elif  (downlink_command.lower() == 'security_mode_command'):
                    response = common.handle_response(pattern,ue_response)
                    full_prompt, llm_answer, status= Security_Mode_Command.security_mode_command_part(factor_to_consider, uplink_command, downlink_command, 
                    response, pcap_file_path,dl_com_invoke_line,renamed_pcap_text,llm_type)
                    
                elif  (downlink_command.lower() == 'configuration_update_command'):
                    response = common.handle_response(pattern,ue_response)
                    full_prompt, llm_answer, status = Configuration_Update_Command.configuration_update_command_part(factor_to_consider, uplink_command, 
                    downlink_command, response, pcap_file_path,dl_com_invoke_line,renamed_pcap_text,llm_type)
                    
                    
                elif  (downlink_command.lower() == 'deregistration_request'):
                    response = common.handle_response(pattern,ue_response)
                    full_prompt, llm_answer, status = Deregistration_Request.deregistration_request_part(factor_to_consider, uplink_command, downlink_command, 
                    response, pcap_file_path,dl_com_invoke_line,renamed_pcap_text,llm_type)
                
                elif  (downlink_command.lower() == 'service_accept'):
                    response = common.handle_response(pattern,ue_response)
                    full_prompt, llm_answer, status = Service_Accept.service_accept_part(factor_to_consider, uplink_command, downlink_command, response, 
                    pcap_file_path,dl_com_invoke_line, renamed_pcap_text,llm_type)
                    
                    
                elif  (downlink_command.lower() == 'service_reject'):
                    response = common.handle_response(pattern,ue_response)
                    full_prompt, llm_answer, status = Service_Reject.service_reject_part(factor_to_consider, uplink_command, downlink_command, response, 
                    pcap_file_path,dl_com_invoke_line, renamed_pcap_text,llm_type)

                elif  (downlink_command.lower() == 'gmm_status'):
                    response = common.handle_response(pattern,ue_response)
                    full_prompt, llm_answer, status = Gmm_Status.gmm_status_part(factor_to_consider, uplink_command, downlink_command, response, 
                    pcap_file_path,dl_com_invoke_line, renamed_pcap_text,llm_type)
                
                elif  (downlink_command.lower() == 'deregistration_accept'):
                    response = common.handle_response(pattern,ue_response)
                    full_prompt, llm_answer, status = Deregistration_Accept.deregistration_accept_part(factor_to_consider, uplink_command, downlink_command, 
                    response, pcap_file_path,dl_com_invoke_line, renamed_pcap_text,llm_type)
                    
                    
                elif  (downlink_command.lower() == 'authentication_reject'):
                    response = common.handle_response(pattern,ue_response)
                    full_prompt, llm_answer, status = Authentication_Reject.authentication_reject_part(factor_to_consider, uplink_command, downlink_command, 
                    response, pcap_file_path,dl_com_invoke_line, renamed_pcap_text,llm_type)
                    
                    
                elif  (downlink_command.lower() == 'registration_reject'):
                    response = common.handle_response(pattern,ue_response)
                    full_prompt, llm_answer, status = Registration_Reject.registration_reject_part(factor_to_consider, uplink_command, downlink_command, response, 
                    pcap_file_path,dl_com_invoke_line, renamed_pcap_text,llm_type)
                ...
                response    = '--'
                status = '--'
                llm_answer = f"{file_name} pacp has insufficient data or uplink or downlink command is missing in the capture"
            
            table_data.append([str(index),file_name, full_prompt,llm_answer])  
            
            # Define the folder and file name
            folder = "saved_prompts"
            ...
            # Save the full_prompt
            with open(file_path, "w", encoding="utf-8") as f:
                f.write(full_prompt)

            print(f"Saved full_prompt to {file_path}")

    
    common.table_creator(table_data,llm_type)
...
if __name__ == "__main__":
    ...
    # Parse arguments
    parser = argparse.ArgumentParser(description="Process pcap and testcase directories.")
    parser.add_argument(
        "mode",
        choices=["chatgpt", "gemini"],
        help="Select mode 'chatgpt' or 'gemini'."
    )
    ...
    main(pcap_directory,testcase_directory, args.mode)
